...
**Changes Made:**
- Added `src/reader/ui/store-controls.ts` which subscribes to store and calls legacy `updateControlsState()`
- Initialized store-bound controls in `src/reader/index.ts` (non-breaking augmentation)
...
## QA Results
**Gate: PASS** — Controls update reactively via store subscription without altering behavior.
