        ...
        if (chunkSize == 1) {
            ...
            if (textOnSlide.indexOf("-") !== -1) {
                if (textOnSlide.length == 1) {
                    ...
                    } else if (madvEnableHyphenatedWordSplit) {
...
function addCommaPauseAfterCharacter(character, text, textItem, splitTextArray) {
    ...
    if (indexCharacter !== -1) {
        ...
        if (indexCharacter == text.length - 1) {
            if (pauseAfterComma) textItem.postdelay = pauseAfterCommaDelay;
            return true;
        }
...
function addPeriodPauseAfterCharacter(character, text, textItem, splitTextArray) {
    ...
    if (indexCharacter) {
        // We have the character at the end
        if (indexCharacter == text.length - 1) {
            if (pauseAfterPeriod) textItem.postdelay = pauseAfterPeriodDelay;
            return true;
        }
        // We have the character at the start
        else if (indexCharacter == 0) {
...
function getTextArrayBasic(selectedText, chunkSize) {
    ...
    for (var i = 0; i < splitText.length; i++) {
        ...
        //console.log("[Sprint Reader] text: " + textItem.text + " (" + textItem.predelay + ", " + textItem.postdelay + ") OLP: " + textItem.optimalletterposition);
...
function getTextArrayBasicTiming(textData) {
    ...
    for (var i = 0; i < textData.length; i++) {
        ...
        // Enable this to get a log of the textItem
        //console.log("[Sprint Reader] text: " + t.text + " (" + t.predelay + ", " + t.duration + ", " + t.postdelay + ") words: " + t.wordsinslide);
    }
}
...
function getTextArrayWordLengthTiming(textData) {
    ...
    for (var i = 0; i < textData.length; i++) {
        ...
        // Enable this to get a log of the textItem
        //console.log("[Sprint Reader] text: " + textData[i].text + " (" + textData[i].predelay + ", " + textData[i].duration + ", " + textData[i].postdelay + ")");
    }
}
...
function playSlideShow() {
    ...
    // Display blank word for postdelay (if timing requires)
        ...
        if (autoCloseReader) doWeAutoCloseReader();
    }

    // Determine if we stop the recursive loop
    if (!stopSlideShow) {
        //console.log("[Sprint Reader] Slide delay: " + showSlideTime + " - textItemIndex: " + textItemIndex-1);
        if (wordFlicker) {
            {
                setTimeout(
                    function () {
        ...
        //console.log("[Sprint Reader] stop command received");
        playingText = false;
    }
}
...
function doWeReverseSlide(textItem) {
    ...
    //console.log('[Sprint Reader] original text ' + textItem.textoriginal);
    //console.log('[Sprint Reader] processed text ' + textItem.text);
...
function highlightTheOptimalLetter(textItem) {
    ...
    if (highlightOptimalLetter && chunkSize == 1) {
...
function calculatePixelOffsetToOptimalCenter(textItem, isRightToLeft) {
    ...
    //console.log('[Sprint Reader] wordToOptimalStart ' + wordToOptimalStart);
    //console.log('[Sprint Reader] wordOptimalLetter ' + wordOptimalLetter);
    //console.log("[Sprint Reader] pxOptimalLetter " + pxOptimalLetter);
    //console.log("[Sprint Reader] pxToOptimalStart " + pxToOptimalStart);
    //console.log("[Sprint Reader] Word: " + textItem.text + " Optimal Letter: " + textItem.optimalletterposition + " Pixel Offset: " + pxOffset);
...
function setSlideShowData(textData) {
    ...
    //console.log('[Sprint Reader] WPM: ' + WPM);
    //console.log('[Sprint Reader] WPM (Real): ' + slideShowData.realWPM);
    //console.log('[Sprint Reader] WPM (All): ' + slideShowData.realWPMAllPauses);
}
...
function displayStatistics(textData) {
    ...
    document.getElementById("stattotalwords").innerHTML = "Total words: <b>" + totalWords + "</b>";
    document.getElementById("stattotalsegments").innerHTML = "Total segments (slides): <b>" + s.totalSegments + "</b>";
    document.getElementById("statmaxduration").innerHTML = "Maximum slide duration: <b>" + s.maxDuration.toFixed() + "ms</b> (" + getMinAndSecondsString(s.maxDuration) + ")";
    document.getElementById("statminduration").innerHTML = "Minimum slide duration: <b>" + s.minDuration.toFixed() + "ms</b> (" + getMinAndSecondsString(s.minDuration) + ")";
    document.getElementById("statavgperslide").innerHTML = "Average slide duration: <b>" + s.avgDuration.toFixed() + "ms</b> (" + getMinAndSecondsString(s.avgDuration) + ")";
    document.getElementById("statchunksize").innerHTML = "Chunk size (words per slide): <b>" + chunkSize + "</b>";
    document.getElementById("statwpm").innerHTML = "Words per minute (WPM): <b>" + WPM + "</b>";
    document.getElementById("stattotaldurationincpause").innerHTML = "Total duration (including pauses): <b>" + s.totalDurationIncPauses.toFixed() + "ms</b> (" + getMinAndSecondsString(s.totalDurationIncPauses) + ")";
    document.getElementById("stattotalduration").innerHTML = "Total duration: <b>" + s.totalDuration.toFixed() + "ms</b> (" + getMinAndSecondsString(s.totalDuration) + ")";
}
...
function adjustWPM(adjustmentAmount) {
    ...
    //console.log('[Sprint Reader] WPM Padding: ' + newPadding + ' | WPM: ' + newWPM + ' | Timing Adjustment: ' + newTimingAdjustment);
